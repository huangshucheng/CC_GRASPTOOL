...
var 用作替换的CK = null
var POST控制= null
var 欲被替换的POST的内容 = null
var 用作替换的POST的内容 = null
var add = null
var IP = null
var change = null
var addcolor = null 
...
class Handlers {
public static RulesOption("隐藏所有状态为 304(无变更) 的请求") BindPref("fiddlerscript.rules.Hide304s") var m_Hide304s: boolean = false;

    // Cause Fiddler to override the Accept-Language header with one of the defined values
public static RulesOption("请求 &日文内容") var m_Japanese: boolean = false;
...
public static ContextAction("★取消所有FiddlerScript规则★") function DoManualReload() {
    FiddlerObject.ReloadScript();
    MessageBox.Show("Fiddler：成功取消以上所有规则", "成功");
}

public static ContextAction("解压缩此条数据（乱码适用）") function DoRemoveEncoding(oSessions: Session[]) {
    ...
    // 自定义替换发送值
public static ContextAction("&替换POST的内容") 
function 替换POST的内容 () {
    欲被替换的POST的内容 = FiddlerObject.prompt("Fiddler：请输入欲被替换的URL", null, "第一步");
    用作替换的POST的内容 = FiddlerObject.prompt("Fiddler：请输入用作替换的URL", null, "第二步");
    POST控制= true
}

public static ContextAction("&替换CK") function 替换CK() {欲被替换CK的域名 = FiddlerObject.prompt("Fiddler：请输入欲被替换的URL", null, "第一步");用作替换的CK = FiddlerObject.prompt("Fiddler：请输入用作替换的URL", null, "第二步");
}

public static ContextAction("&替换URL") function 替换URL() {欲被替换的的URL = FiddlerObject.prompt("Fiddler：请输入欲被替换的URL", null, "第一步");用作替换的的URL = FiddlerObject.prompt("Fiddler：请输入用作替换的URL", null, "第二步");
}

    //  
    // 自定义替换返回值
public static ContextAction("&替换返回内容") function ChangeResponsebody() {
    欲被替换的返回内容 = FiddlerObject.prompt("Fiddler：请输入欲被替换的返回内容", null, "第一步");
    用作替换的返回内容 = FiddlerObject.prompt("Fiddler：请输入用作替换的返回内容", null, "第二步");
}
public static ContextAction("&域名自动标红加粗") function Changecolor() {
    addcolor = FiddlerObject.prompt("Fiddler：请输入需要自动标红的域名", null, "第一步");
}
public static ContextAction("&伪装IP") function ChangeIP() {
    add = FiddlerObject.prompt("Fiddler：请输入需要伪装的域名", null, "第一步");
    IP = FiddlerObject.prompt("Fiddler：请输入伪装的IP 留空则随机IP", null, "第二步");
}
static
function OnBeforeRequest(oSession: Session) {
    ...
    // }
    if (oSession.uriContains(addcolor)) {
    ...
    if (add == "") {
        ...
        if (oSession.uriContains(add)) {
            oSession.oRequest.headers.Add("Cient_ip", IP);
            oSession.oRequest.headers.Add("X-forwarded-for", IP);
        }
    }
    ////////////////////////////////////////////////////////////////////自定义区
    if (oSession.uriContains(欲被替换CK的域名)) { oSession.oRequest.headers["Cookie"] = 用作替换的CK}
    if (oSession.utilFindInResponse(欲被替换的返回内容, false) != -1) { oSession.utilReplaceInResponse(欲被替换的返回内容, 用作替换的返回内容)} 
        if(URL控制==true){oSession.url = oSession.url.Replace(欲被替换的的URL, 用作替换的的URL) }
    if(POST控制 == true) {oSession.utilReplaceInRequest(欲被替换的POST的内容, 用作替换的POST的内容);}
    ////////////////////////////////////////////////////////////////////自定义区
    ...
    if(oSession.uriContains(".")){
        //设置上方为域名
        //下方为伪装IP
        var ip="117.136.4."+GetRandomNum(0,255);
        oSession.oRequest.headers.Add("Cient_ip", ip);
        oSession.oRequest.headers.Add("X-forwarded-for", ip);
    }
...
function OnBeforeResponse(oSession: Session) {
    ...
    if (oSession.utilFindInResponse("get_brand_wcpay_request:ok", false) != -1) {
        oSession.utilReplaceInResponse("get_brand_wcpay_request:ok", "get_brand_wcpay_request:cancel") 
        FiddlerObject.log("已检测到可能的跳过点，请在支付页面返回，查看结果")
    }
...
static
function OnBoot() {
    UI.ActivateRequestInspector("webforms");
    UI.ActivateResponseInspector("raw");
    FiddlerObject.log("欢迎使用FD \n★东东定制版Fiddler★\n1、指定域名伪装IP（可随机）\n2、域名自动标注\n3、添加自定义修改返回值菜单\n4、微信全自动检测并跳过支付")
}
...
BindPref("fiddlerscript.ephemeral.bpRequestURI") public static
var bpRequestURI: String = null;

BindPref("fiddlerscript.ephemeral.bpResponseURI") public static
var bpResponseURI: String = null;

BindPref("fiddlerscript.ephemeral.bpMethod") public static
