function main()
    -- unicode编码到UTF8编码
    notifyMessage(Unicode2Utf8("\\u54c8\\u54c8"))
    mSleep(2000)
    -- UTF8编码到Unicode编码-大写U
    notifyMessage(Utf82Unicode("哈哈",true))
    -- UTF8编码到Unicode编码-小写U
    notifyMessage(Utf82Unicode("哈哈",false))
...
function s2h(str,spacer)return (string.gsub(str,"(.)",function (c)return string.format("%02x%s",string.byte(c), spacer or"")end))end
function h2s(h)return(h:gsub("(%x%x)[ ]?",function(w)return string.char(tonumber(w,16))end))end 
